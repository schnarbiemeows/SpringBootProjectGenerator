...
class PojoAndDtoGenerator:
    ...
    def create_pojo_and_dto_classes(self, table, src, tabledata):
        """
        ...
        if src == "pojo":
            ...
            if len(table.primarykeys)>1:
                ...
                self.make_primary_key_class(table,pkfilename,src)
        else:
        ...
        # create the fields
        self.create_the_fields(table, resources_file,src, tabledata)
        # create the default constructor
        self.create_def_constr(table, resources_file,src)
        # create the field constructor
        self.create_field_constr(table, resources_file,src, False,tabledata)
        # create the getters and setters
        self.create_get_n_set(table,resources_file,src)
        # create the toString
        self.create_tostring(table, resources_file,src,tabledata)
        ...
        self.create_dto_pojo_conv(table, resources_file, src,tabledata)
        # finish the class with a trailing }
        resources_file.write("}\n")
        resources_file.close()

    def make_primary_key_class(self, table, pkfilename, src):
        """
        ...
        for field in table.fieldnames:
            ...
            if fielddata.isprimary == True:
                # create the field
                self.create_field(fielddata, resources_file)
                # make the getter
                resources_file.write(
        ...
        self.create_def_constr(table, resources_file, src)
        self.create_field_constr(table, resources_file,src,True)
        self.createEqandHashCde(table,resources_file)
        resources_file.write("}\n")
        resources_file.close()
    ...
    def create_the_fields(self, table, file, src, tabledata):
        """
        ...
        if src == "pojo":
            ...
            if compoundPK == True:
                self.create_object_javadoc("compound primary key",file)
                file.write(tabs+"@EmbeddedId\n")
                file.write(tabs+"private "+table.camelcasejavaname+"PK compoundKey;\n\n")
                self.create_get_n_set_for_pk(table, file, True)
            else:
                for field in table.fieldnames:
                    fielddata = table.fielddata[field]
                    if fielddata.isprimary == True:
                        ...
                        self.create_get_n_set_for_pk(table, file, False)
            ...
            if len(table.fksymbolnames)>0:
                ...
                for symbolname in table.fksymbolnames:
                    ...
                    self.create_object_javadoc(parenttable.camelcasejavaname+" foreign key", file)
                    ...
                    self.create_object_javadoc(parenttable.camelcasejavaname + " foreign key getter", file)
                    file.write(
                        tabs + "public " + parenttable.camelcasejavaname + " get" + parenttable.camelcasejavaname + "() {\n" + tabs + tabs + "return " + parenttable.lowercasename + ";\n" + tabs + "}\n\n")
                    # make the setter
                    self.create_object_javadoc(parenttable.camelcasejavaname + " foreign key setter", file)
                    file.write(
            ...
            if len(table.parentkeysymbolnames) > 0:
                ...
                for symbolname in table.parentkeysymbolnames:
                    ...
                    relationship = ''
                    self.create_object_javadoc(childtable.camelcasejavaname + " child key", file)
                    if childfield.isprimary == True:
                        ...
                        if childfield.unique == False:
                            ...
                            self.create_object_javadoc(childtable.camelcasejavaname + " child key getter", file)
                            file.write(
                            ...
                            # make the setter
                            self.create_object_javadoc(childtable.camelcasejavaname + " child key setter", file)
                            file.write(
                            ...
                            self.create_object_javadoc(childtable.camelcasejavaname + " add a child key", file)
                            file.write(
                            ...
                            # make the setter
                            self.create_object_javadoc(childtable.camelcasejavaname + " remove a child key", file)
                            file.write(
    ...
    def create_object_javadoc(self,msg,file):
        """
    ...
    def create_def_constr(self, table, file, src):
        ...
        """
        tabs = Constants.tab
        self.create_object_javadoc("default constructor", file)
        if src == "pojo":
    ...
    def create_field_constr(self, table, file, src, embeddedPK, tables=None):
        """
        create the field constructor
        :param table:
        :param file:
        ...
        tabs = Constants.tab
        text = ""
        self.create_object_javadoc("field constructor", file)
        if src == "pojo":
    ...
    def create_get_n_set_for_pk(self, table, file, compoundPk=False):
        """
        ...
        tabs = Constants.tab
        if compoundPk == True:
            self.create_object_javadoc("compound primary key getter", file)
            file.write(
                tabs + "public " + table.camelcasejavaname + "PK getCompoundKey() {\n" + tabs + tabs + "return this.compoundKey;\n" + tabs + "}\n\n")
            # make the setter
            self.create_object_javadoc("compound primary key setter", file)
            file.write(
            ...
            for field in table.fieldnames:
                fielddata = table.fielddata[field]
                if fielddata.isprimary == True:
                    self.create_object_javadoc("primary key getter", file)
                    file.write(
                        tabs + "public " + fielddata.datatype + " get" + fielddata.gettername + "() {\n" + tabs + tabs + "return " + fielddata.javaname + ";\n" + tabs + "}\n\n")
                    # make the setter
                    self.create_object_javadoc("primary key getter", file)
    ...
    def create_get_n_set(self, table, file, src):
        """
        create the getters and setters
        :param table:
        :param file:
        ...
        if src == "pojo":
            for field in table.fieldnames:
                ...
                if fielddata.isforeignkey == False and fielddata.isprimary == False:
                    self.create_object_javadoc(fielddata.javaname+" getter",file)
                    file.write(
                        tabs + "public " + fielddata.datatype + " get" + fielddata.gettername + "() {\n" + tabs + tabs + "return " + fielddata.javaname + ";\n" + tabs + "}\n\n")
                    self.create_object_javadoc(fielddata.javaname+" setter", file)
                    file.write(
                        tabs + "public void set" + fielddata.gettername + "(" + fielddata.datatype + " " + fielddata.javaname + ") {\n" + tabs + tabs + "this." + fielddata.javaname + "=" + fielddata.javaname + ";\n" + tabs + "}\n\n")
        else:
            # FOR EACH FIELD:
    ...
    def create_tostring(self, table, file, src, tabledata):
        """
    ...
    def create_dto_pojo_conv(self, table, file, src, tabledata):
        """
    ...
    def createEqandHashCde(self,table,file):
        """
